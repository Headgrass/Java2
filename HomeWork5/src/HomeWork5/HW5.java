...
public class HW5 {
    ...
    public static long makeArr2() {
        ...
        System.arraycopy(arr1,0,arr,9,HALF);
        System.arraycopy(arr2,0,arr,HALF,HALF);

        return System.currentTimeMillis()-a;
    }

    public static float[] calculateArr(float[] arr, int startNum) {
        for (int i = 0; i < arr.length; i++) {
            arr[i] = (float) (arr[i] * Math.sin((startNum + i)/5) * Math.cos(0.2f + (startNum +i)/5)* Math.cos(0.4f + i) / 2);}
            return arr;
        }
    }
